...
for root in "${SEARCH_ROOTS[@]}"; do
  ...
  while IFS= read -r dir; do
    ...
    find "$root" -maxdepth 5 -type d \
      \( -name 'prescrivia' -o -name 'leverwork' -o -name 'jsvcapital' -o -name 'sonopeace' -o -name 'clawd' -o -name 'aeoa-studio' -o -name 'nilsy-astro' -o -name 'openclaw-computer-vision' \) \
      2>/dev/null
  )
done
