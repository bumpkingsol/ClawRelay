...
- ✅ A1 implemented: `/context/meetings` now derives `has_transcript` in SQL without selecting transcript body.
- ✅ B1 implemented: index added for `meeting_sessions(summary_purge_at)`.
- ✅ B2 implemented: expression index added for `participant_profiles(COALESCE(last_seen, last_updated))`.
- ✅ C1-C3 implemented: `meeting_participants` table + indexes, dual-write on session upsert, and `detect_patterns()` migrated to indexed participant lookup (with legacy fallback).
- ⏳ Remaining: A2, B3, D1-D3.
