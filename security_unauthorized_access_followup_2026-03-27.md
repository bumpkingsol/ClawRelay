...
- Evidence:
  - `https://<SERVER_IP>:7890/context/push` timed out from this Mac
  - raw TCP checks to `<SERVER_IP>` on ports `7890`, `443`, and `80` also timed out
...
- Recommendation: copy `/home/user/clawrelay/data/certs/context-bridge.pem` from the server to `~/.context-bridge/server-ca.pem`, or rerun the installer with the PEM path.
