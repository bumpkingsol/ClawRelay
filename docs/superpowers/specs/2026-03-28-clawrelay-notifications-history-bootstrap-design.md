...
1. **Handoff status change** — JC marks a handoff as "in-progress" or "done"
   - Example: "JC started: fix deploy script (project-alpha)"
   - Example: "JC completed: PR review (project-alpha)"
...
3. **JC question** — JC posts a question that needs Jonas's attention
   - Example: "JC asks: Should I pick up the auth migration on project-alpha?"
...
JC writes a question. Requires Bearer auth. Body: `{"question": "Should I pick up X?", "project": "project-alpha"}`
