...
DIGEST_DIR = Path(
    os.environ.get(
        "CONTEXT_BRIDGE_DIGEST_DIR",
        str(Path(DB_PATH).parent / "memory" / "activity-digest"),
    )
)
REPOS_DIR = Path(os.environ.get("CLAWRELAY_REPOS_DIR", str(Path(DB_PATH).parent)))

# Validate paths exist before proceeding
if not REPOS_DIR.exists():
    logger.error(f"REPOS_DIR {REPOS_DIR} does not exist")
    sys.exit(1)
...
def get_project_last_seen(db_path):
    ...
    conn = sqlite3.connect(db_path)
    rows = conn.execute(
        "SELECT project, last_seen, last_branch FROM project_last_seen"
    ).fetchall()
    conn.close()
...
def get_recent_commits(since_ts):
    ...
    db = get_db()
    commits = db.execute(
        "SELECT * FROM commits WHERE ts >= ? ORDER BY ts", (since_ts,)
    ).fetchall()
    db.close()
    return commits
...
def build_digest(hours_back=8):
    ...
    rows = db.execute(
        "SELECT * FROM activity_stream WHERE ts >= ? ORDER BY ts", (since,)
    ).fetchall()
    ...
    for row in rows:
        ...
        if row["whatsapp_context"]:
            details["whatsapp"].append(
                {"ts": row["ts"], "context": row["whatsapp_context"]}
            )
        ...
        if row.get("notifications"):
            details["notifications"].append(
                {"ts": row["ts"], "data": row["notifications"]}
            )
        ...
        if row.get("calendar_events"):
            details["calendar_events"].append(
                {"ts": row["ts"], "events": row["calendar_events"]}
            )
        ...
        if row["codex_session"]:
            all_codex_sessions.append(
                {"ts": row["ts"], "session": row["codex_session"]}
            )
    ...
    prev_since = (
        datetime.fromisoformat(since) - timedelta(hours=hours_back)
    ).isoformat()
    prev_until = since
    ...
    current_projects = set(
        p for p, c in project_captures.items() if c > 0 and p != "other"
    )
    ...
    for p in sorted(PORTFOLIO_PROJECTS.keys()):
        if p in current_projects:
            ...
            try:
                last = datetime.fromisoformat(last_seen_data[p]["last_seen"]).replace(
                    tzinfo=timezone.utc
                )
                days_ago = (datetime.now(timezone.utc) - last).days
    ...
    try:
        ...
        if meetings:
            ...
            for m in meetings:
                ...
                start_time = (
                    m["started_at"][11:16]
                    if m["started_at"] and len(m["started_at"]) > 16
                    else "??:??"
                )
                end_time = (
                    m["ended_at"][11:16]
                    if m["ended_at"] and len(m["ended_at"]) > 16
                    else "??:??"
                )
                ...
                try:
                    participants = (
                        ...
                        if isinstance(m["participants"], str)
                        else (m["participants"] or [])
                    )
                ...
                meeting_title = (
                    m["id"].replace("-", " ").title() if m["id"] else "Unknown"
                )
                lines.append(
                    f"- **{start_time}-{end_time}** {meeting_title} ({app_name}, {participant_str})"
                )
    ...
    for proj, count in sorted(project_captures.items(), key=lambda x: -x[1]):
        ...
        if details["in_call_minutes"] > 0:
            call_apps_str = (
                ", ".join(details["call_apps"]) if details["call_apps"] else "unknown"
            )
            lines.append(
    ...
    if all_whatsapp_messages:
        ...
        for chat_label, chat_data in chats.items():
            ...
            if timestamps:
                first = (
                    timestamps[0].split("T")[1][:5]
                    if "T" in timestamps[0]
                    else timestamps[0]
                )
                last = (
                    timestamps[-1].split("T")[1][:5]
                    if "T" in timestamps[-1]
                    else timestamps[-1]
                )
                time_range = f", {first}–{last}"

            lines.append(
                f"**{chat_label}** ({chat_data['id']}) — {len(msgs)} messages{time_range}"
            )
            lines.append("")
    ...
    if all_notifications:
        ...
        for entry in all_notifications:
            try:
                ...
                for n in notifs[:3]:
                    lines.append(
                        f"- {entry['ts']}: {n.get('app', '?')} — {n.get('title', '')}"
                    )
            except:
                pass
...
def main():
    ...
    parser.add_argument(
        "--window",
        type=str,
        choices=["last-4h", "4-12h", "yesterday"],
        default=None,
        help="Digest window (non-overlapping)",
    )
    args = parser.parse_args()

    if args.window:
        now = datetime.now(timezone.utc)
        if args.window == "last-4h":
            hours_back = 4
        elif args.window == "4-12h":
            since = (now - timedelta(hours=12)).isoformat()
            rows = (
                get_db()
                .execute(
                    "SELECT COUNT(*) as cnt FROM activity_stream WHERE ts >= ?",
                    (since,),
                )
                .fetchone()["cnt"]
            )
            if rows > 0:
                ...
                logger.info("No data in 4-12h window, skipping")
                return
        elif args.window == "yesterday":
            ...
            if rows > 0:
                ...
                logger.info("No data yesterday, skipping")
                return
    else:
        hours_back = args.hours

    digest = build_digest(hours_back)
